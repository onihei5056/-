...
- **クイック変更**：比較エリアの下のボタン（壁紙を白系に／アクセントクロス／床を明るく／床をダーク系に／ダウンライト風／全体を新しく）を押すと、**表示中の1枚だけ**を作り直します。設定パネルへ戻る必要がありません
...
**主な利用環境はPC**を想定しています。スマートフォンは
「出先（内見中）で、壁紙や床だけを変えたイメージをその場でお客様に見せる」
という限定的な用途に絞って設計しています。

そのため、スマホでの中心的な操作は**設定パネルを一から埋めることではなく**、
事務所で作成済みの案件を履歴から開き、**クイック変更のボタンを1タップ押すこと**です。

```
出先での想定操作（3タップ）
  履歴 → 案件をタップして開く → 「壁紙を白系に」を1タップ → 画像が差し替わる
```

そのうえで、新規に撮影して作る場合にも対応しています。

| 対応内容 | 内容 |
|---|---|
| クイック変更 | 結果画面から1タップで内装を変更し、表示中の1枚だけを作り直す（出先での主動線。PCでも利用可） |
| 登録済み物件から選ぶ | アップロード欄から履歴へ移動し、事務所で登録済みの写真をそのまま使える |
| 設定項目の折りたたみ | スマホではリフォーム／ターゲット／追加の希望を折りたたみ、既定のスタイルも1つに絞ってスクロール量を削減 |
