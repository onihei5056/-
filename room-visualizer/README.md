...
PCの3カラムをそのまま縮小するのではなく、**内見現場での利用**を前提にスマホ専用の導線を用意しています。

| 対応内容 | 内容 |
|---|---|
| ボトムナビゲーション | 親指の届く画面下部にホーム／生成／お気に入り／履歴／設定を配置（640px以下で表示） |
| 2タブ構成 | 画像生成画面を「条件を設定」「生成結果」に分割。生成が終わると自動で結果タブへ切り替わります |
| カメラ起動 | 「カメラで撮影する」で背面カメラを直接起動（`capture="environment"`）。「写真ライブラリから選ぶ」と分けてあります |
| HEIC対応 | iPhoneの写真（HEIC/HEIF）も受け付け、JPEGへ自動変換します |
| 生成ボタン固定 | 「AIで生成する」を画面下部に固定表示し、長い設定を最後までスクロールしなくても押せます |
| 全画面プレビュー | 生成画像を1タップで全画面表示。お客様にその場で見せる用途を想定しています |
| セーフエリア | ノッチ／ホームインジケーターを `env(safe-area-inset-*)` で回避（`viewport-fit=cover`） |
| 入力欄の自動ズーム防止 | iOS Safariが入力欄フォーカス時に拡大しないよう、スマホでは文字サイズを16pxにしています |
| タップ領域 | ボタン・チェックボックスを最低40〜44pxに拡大 |
| Before/Afterスライダー | つまみを掴んだときだけドラッグ開始。画像上での上下スクロールを妨げません |
| 履歴画面 | 元画像をサムネイル化し、絞り込み条件は「絞り込み」ボタンで開閉 |

**ホーム画面への追加**：Safariの共有メニューから「ホーム画面に追加」すると、
アドレスバーのない全画面表示で起動します（`apple-mobile-web-app-capable`）。
ただし現時点ではオフライン動作はしません。オフライン対応が必要な場合は、
`app/`（現地調査PWA）と同様に `vite-plugin-pwa` を追加してください。

> **社内スマホから使うには HTTPS 配信が必要です。**
> `npm run dev` のローカルURLは同一Wi-Fi内の実機からも開けますが（`http://<PCのIP>:5174/`）、
> **カメラ起動は HTTPS でないと動作しません**。実機で試す場合はHTTPSで配信してください。
...
- ログイン・ユーザー管理はありません。
- スマートフォンではオフライン動作しません（Service Worker未導入）。
- 写真の向き（EXIF Orientation）はブラウザ標準の挙動に依存します（iOS Safari 13.4以降／Chrome 81以降で正しく表示されます）。
